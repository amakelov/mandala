...
import textwrap
from utils import get_nullable_union, get_setdict_union, get_setdict_intersection, get_dict_union_over_keys, get_dict_intersection_over_keys, get_adjacency_union, get_adjacency_intersection, get_adj_from_edges, get_nullable_intersection
...
class ComputationFrame:
    ...
    def add_func(self, fname: Optional[str],) -> str:
        if fname is None:
            fname = self.get_new_fname("f")
        logging.debug(f"Adding function {fname}, called from {inspect.stack()[1].function}")
        self.fs[fname] = set()
        self.inp[fname] = {}
        self.out[fname] = {}
        return fname
    ...
    def edges(self) -> List[Tuple[str, str, str]]:
        return [(src, dst, label) for src, dsts_dict in self.out.items() for label, dsts in dsts_dict.items() for dst in dsts]
    ...
    def drop_ref(self, vname: str, hid: str):
        ...
        if len(self.refinv[hid]) == 1: # if this is the only variable containing the ref
            del self.refs[hid]
            del self.refinv[hid]
    ...
    def drop_call(self, fname: str, hid: str):
        if len(self.callinv[hid]) == 1: # if this is the only function containing the call
            call = self.calls[hid]
            del self.calls[hid]
    ...
    def get_source_elts(self) -> Dict[str, Set[str]]:
        return {node: {hid for hid in self.sets[node] if hid not in self.creator} for node in self.nodes}
    ...
    def get_sink_elts(self) -> Dict[str, Set[str]]:
        return {node: {hid for hid in self.sets[node] if hid not in self.consumers} for node in self.nodes}
    ...
    def get_adj_elts_edge(self, 
                          edge: Tuple[str, str, str], 
                          hids: Set[str],
                          direction: Literal["back", "forward"]) -> Set[str]:
        """
        ...
        if direction == "back":
            if dst in self.vs: # this is a variable node; we are looking for the calls that created the hids
                assert hids <= self.vs[dst] # make sure the hids are valid
                return {self.creator[hid] for hid in hids if hid in self.creator} & self.fs[src]
                ...
                return self.vs[src] & {self.calls[call_hid].inputs[name].hid for call_hid in hids 
                                       for name in self.get_names_projecting_to(call_hid, label)}
        else: # direction == "forward"
            if src in self.vs: # this is a variable node; we are looking for the calls that consume the hids
                assert hids <= self.vs[src]
                return {call_hid for call_hid in self.fs[dst] if 
                        any(self.calls[call_hid].inputs[name].hid in hids 
                        for name in self.get_names_projecting_to(call_hid, label))}
            else: # this is an op node; we are looking for the outputs of the calls that have the hids as inputs
                assert hids <= self.fs[src]
                return {self.calls[call_hid].outputs[name].hid 
                        for call_hid in hids 
                        for name in self.get_names_projecting_to(call_hid, label)
                        if name in self.calls[call_hid].outputs} & self.vs[dst]
    
    def get_adj_elts(self, node: str, hids: Set[str], direction: Literal["back", "forward", "both"]) -> Dict[str, Set[str]]:
        """
        ...
        if direction in ["back", "both"]:
            ...
            res.update({src: self.get_adj_elts_edge((src, dst, label), hids, "back") for (src, dst, label) in edges})
        if direction in ["forward", "both"]:
            edges = self.out_edges(node)
            res.update({dst: self.get_adj_elts_edge((src, dst, label), hids, "forward") for (src, dst, label) in edges})
        return res
    ...
    def select_nodes(self, nodes: Iterable[str]) -> "ComputationFrame":
        ...
        res = self.copy()
        nodes = set(nodes)
        edges = {(src, dst, label) for src, dst, label in res.edges() if (src in nodes and dst in nodes)}
        ...
        refinv = {hid: vnames & set(nodes) for hid, vnames in res.refinv.items() if hid in ref_hids}
        callinv = {hid: fnames & set(nodes) for hid, fnames in res.callinv.items() if hid in call_hids}
        creator = {ref_hid: call_hid for ref_hid, call_hid in res.creator.items() if ref_hid in ref_hids and call_hid in call_hids}
        consumers = {ref_hid: (consumer_call_hids & call_hids) for ref_hid, consumer_call_hids in res.consumers.items() if ref_hid in ref_hids}
        return ComputationFrame(storage=res.storage,
                                inp=inp, out=out, vs=vs, fs=fs,
                                refinv=refinv, callinv=callinv, creator=creator,
                                consumers=consumers, refs=refs, calls=calls)
    ...
    def select_subsets(self, elts: Dict[str, Set[str]]) -> "ComputationFrame":
        ...
        creator = {hid: res.creator[hid] for hid in ref_hids_subset if hid in res.creator and res.creator[hid] in call_hids_subset}
        consumers = {hid: res.consumers[hid] & call_hids_subset for hid in ref_hids_subset if hid in res.consumers}
        return ComputationFrame(storage=res.storage,
                                inp=inp, out=out, vs=vs, fs=fs,
                                refinv=refinv, callinv=callinv, creator=creator,
                                consumers=consumers, refs=refs, calls=calls)
    ...
    def downstream(self, *nodes: str) -> "ComputationFrame":
        ...
        downstream_view = res.get_reachable_elts(initial_state={node: res.sets[node] for node in nodes}, 
    ...
    def upstream(self, *nodes: str) -> "ComputationFrame":
        ...
        upstream_view = res.get_reachable_elts(initial_state={node: res.sets[node] for node in nodes}, 
                                direction='back', how="strong")
    ...
    ############################################################################ 
    @staticmethod
    def _binary_union(a: "ComputationFrame", b: "ComputationFrame") -> "ComputationFrame":
        """
        ...
        res = ComputationFrame(storage=a.storage, inp=inp, out=out, vs=vs, fs=fs, refinv=refinv, callinv=callinv, creator=creator, consumers=consumers, refs=refs, calls=calls)
        res._check() #! not every union results in a valid CF
        return res
    
    @staticmethod
    def _binary_intersection(a: "ComputationFrame", b: "ComputationFrame") -> "ComputationFrame":
        """
    ...
    @staticmethod
    def _binary_setwise_difference(a: "ComputationFrame", b: "ComputationFrame") -> "ComputationFrame":
        """
    ...
    @staticmethod
    def _binary_difference(a: "ComputationFrame", b: "ComputationFrame") -> "ComputationFrame":
        """
    ...
    def _group_calls(self, 
                     ...
                     by: Literal['inputs', 'outputs']) -> Dict[Tuple[str, Tuple[Tuple[str, Tuple[str,...]]]], List[Call]]:
        """
        ...
        for call in calls:
            ...
            group_id = (op_id, tuple(sorted((label, tuple(sorted(variables))) for label, variables in identifying_variables.items())))
            if group_id not in call_groups:
                call_groups[group_id] = []
            call_groups[group_id].append(call)
        return call_groups
    
    def _expand_from_call_groups(self,
        ...
        for group_id, group_calls in sorted(call_groups.items()):
            ...
            label_to_connected_vnames = {label: vnames for label, vnames in connected_vnames_tuple}
            ...
            if side_to_glue == 'outputs':
                ...
                for call in group_calls:
                    ...
                    dual_side_labels_list.append(set([name_projector(name) for name in call.inputs]))
                ...
                for call in group_calls:
                    name_projector = get_name_proj(call.op)
                    dual_side_labels_list.append(set([name_projector(name) for name in call.outputs]))
                dual_side_labels = get_nullable_union(*dual_side_labels_list)
    ...
    def expand_back(self, 
        ...
        res._expand_from_call_groups(call_groups, side_to_glue='outputs',)
        return res
    ...
    def expand_forward(self,
        ...
        res._expand_from_call_groups(call_groups, side_to_glue='inputs',)
        return res
    ...
    def eval(self, *nodes: str, values: Literal["refs", "objs"] = "objs", verbose: bool = True) -> pd.DataFrame:
        """
    ...
    def get_df(self, *nodes:str, values: Literal["refs", "objs"] = "objs", verbose: bool = True,
               include_calls: bool = False) -> pd.DataFrame:
        """
        ...
        if verbose:
            print(f"Extracting tuples from the computation graph:\n{cf.get_graph_desc()}...")
        vnames = {x for x, sink_elts in cf.get_sink_elts().items() if x in cf.vnames and sink_elts}
        df = cf.get_joint_history_df(vnames=vnames, how='outer', include_calls=include_calls)[list(nodes)]
        if values == "refs":
            res = df
        elif values == "objs":
            res = cf.eval_df(df)
    ...
    def get_func_table(self, fname: str) -> pd.DataFrame:
        ...
        for call_uid in self.fs[fname]:
            ...
            call_data = {}
            for input_name, input_ref in call.inputs.items():
                call_data[input_name] = input_ref if input_ref.hid in self.refs else None
            for output_name, output_ref in call.outputs.items():
                call_data[output_name] = output_ref if output_ref.hid in self.refs else None
            rows.append(call_data)
        return pd.DataFrame(rows)
    
    @staticmethod
    def _unify_subobjects(a: Dict[str, Set[str]], b: Dict[str, Set[str]]) -> Dict[str, Set[str]]:
        return {k: a.get(k, set()) | b.get(k, set()) for k in a.keys() | b.keys()}
    
    @staticmethod
    def _is_subobject(a: Dict[str, Set[str]], b: Dict[str, Set[str]]) -> bool:
        return all([a[k] <= b[k] for k in a.keys() & b.keys()]) and all([k in b for k in a.keys()])
    
    def get_direct_history(self, node: str, hids: Set[str], 
                           include_calls: bool
                           ) -> Dict[str, Set[str]]:
        """
        ...
        for hid in hids:
            ...
            for creator_fname in creator_fnames:
                if include_calls:
                    if creator_fname not in res: res[creator_fname] = set()
                ...
                for edge_source, _, edge_label in creator_node_in_edges:
                    ...
                    for inv_input_name in inv_proj_input_names:
                        if creator_call.inputs[inv_input_name].hid in self.vs[edge_source]:
                            if edge_source not in res: res[edge_source] = set()
                            res[edge_source].add(creator_call.inputs[inv_input_name].hid)
            total_res = ComputationFrame._unify_subobjects(total_res, res)
        return total_res
    
    def get_total_history(self, node: str, 
                          hids: Set[str],
                          result: Optional[Dict[str, Set[str]]] = None,
                          include_calls: bool = False
                          ) -> Dict[str, Set[str]]:
        direct_history = self.get_direct_history(node, hids, include_calls=include_calls)
        if result is not None and ComputationFrame._is_subobject(direct_history, result):
            return result
        else:
            ...
            for node in direct_history.keys():
                if node not in self.vnames:
                    continue
                total_res = ComputationFrame._unify_subobjects(total_res, self.get_total_history(node, direct_history[node], result=total_res, include_calls=include_calls))
            return total_res
    
    def get_history_df(self, vname: str, include_calls: bool = False) -> pd.DataFrame:
        rows = []
        for history_id in self.vs[vname]:
            total_history = self.get_total_history(vname, {history_id}, include_calls=include_calls)
            total_history_objs = {vname: {self.refs[ref_uid] for ref_uid in ref_uids} for vname, ref_uids in total_history.items()
                                  if vname in self.vnames}
            total_history_calls = {} if not include_calls else {fname: {self.calls[call_hid] for call_hid in call_hids} for fname, call_hids in total_history.items()
                                    if fname in self.fnames}
            # unpack singletons
            total_history_objs = {k: list(v)[0] if len(v) == 1 else v for k, v in itertools.chain(total_history_objs.items(), total_history_calls.items())}
            rows.append(total_history_objs)
        df = pd.DataFrame(rows)
        return self._sort_df(df)
    
    def get_joint_history_df(self, vnames: Iterable[str], how: Literal["inner", "outer"] = "outer",
                             include_calls: bool = False) -> pd.DataFrame:
        
        def extract_hids(x: Union[None, Ref, Call, Set[Ref], Set[Call]]) -> Union[None, str, Set[str]]:
            if pd.isnull(x):
                return None
        ...
        def eval_hids(hids: Union[None, str, Set[str]]) -> Union[None, Ref, Call, Tuple[Union[Ref, Call]]]:
            if pd.isnull(hids):
                return None
            elif isinstance(hids, str):
                return self.refs[hids] if hids in self.refs else self.calls[hids]
            else:
                return tuple(sorted({self.refs[hid] if hid in self.refs else self.calls[hid] for hid in hids}, key=lambda x: x.hid))

        history_dfs = [self.get_history_df(vname, include_calls=include_calls).applymap(extract_hids) for vname in vnames]
        result = history_dfs[0]
        for df in history_dfs[1:]:
            shared_cols = set(result.columns) & set(df.columns)
            result = pd.merge(result, df, how=how, on=list(shared_cols), suffixes=("", ""))
        # go back to refs
    ...
    def values(self) -> Dict[str, Union[Set[Ref], Set[Call]]]:
        ...
        op_values = {fname: {self.calls[call_hid] for call_hid in call_hids} for fname, call_hids in self.fs.items()}
        return {**var_values, **op_values}
    
    def apply(self, f: Callable, to: Literal["refs", "vals"] = "vals") -> "ComputationFrame":
        """
        ...
        if to == "refs":
            ...
            res.refs = {hid: f(res.storage.unwrap(ref)) for hid, ref in res.refs.items()}
        else:
    ...
    def copy(self) -> "ComputationFrame":
        return ComputationFrame(storage=self.storage,
                                inp=copy.deepcopy(self.inp), out=copy.deepcopy(self.out),
                                vs=copy.deepcopy(self.vs), fs=copy.deepcopy(self.fs),
                                refinv=copy.deepcopy(self.refinv), callinv=copy.deepcopy(self.callinv),
                                creator=copy.deepcopy(self.creator), consumers=copy.deepcopy(self.consumers),
                                refs={k: v for k, v in self.refs.items()}, calls={k: v for k, v in self.calls.items()})
    
    def drop(self, nodes: Iterable[str], inplace: bool = False, 
             with_dependents: bool = False,) -> "ComputationFrame":
        if with_dependents:
            raise NotImplementedError()
        res = self if inplace else self.copy()
        for node in nodes:
    ...
    def __getitem__(self, indexer: Union[str, Iterable[str], "ComputationFrame"]) -> "ComputationFrame":
    ...
    def get_reachable_elts(self, initial_state: Dict[str, Set[str]], 
                           how: Literal["strong", "weak"],
                           direction: Literal["forward", "back"],
                           pad: bool = True) -> Dict[str, Set[str]]:
        """
        ...
        for node in node_order:
            ...
            #     continue
            adj_edges = self.in_edges(node) if direction == "forward" else self.out_edges(node)
            ...
            for edge in adj_edges:
                ...
                adj_elts.append(self.get_adj_elts_edge(edge=edge, hids=hids, direction=direction))
    ...
    def __lt__(self, other: Union["ComputationFrame", Any]) -> "ComputationFrame":
        if isinstance(other, ComputationFrame):
            ...
            vars_view = {vname: {hid for hid in self.vs[vname] if self.storage.unwrap(self.refs[hid]) < other} for vname in self.vnames}
            full_view = self._add_adj_calls(vars_view=vars_view)
            return self.select_subsets(full_view)
    
    def isin(self, values: Any, by: Literal["hid", "cid", "val"] = "val", 
             node_class: Literal["var", "func"] = "var"
             ) -> "ComputationFrame":
        subsets = {}
        if node_class != "var": raise NotImplementedError()
        # first, gather the variable hids
        for vname, hids in self.vs.items():
            if by == "val":
                subsets[vname] = {hid for hid in hids if self.storage.unwrap(self.refs[hid]) in values}
            elif by == "hid":
    ...
    ### ummmm... more stuff
    ############################################################################ 
    def merge_into(self, node_to_merge: str, merge_into: str, inplace: bool = False) -> "ComputationFrame":
        """
        ...
        if not res.sets[node_to_merge] <= res.sets[merge_into]:
            raise ValueError(f"Node {node_to_merge} is not a subset of node {merge_into}")
    ...
    def split(self, var: str, ):
        raise NotImplementedError()
    
    def drop_unreachable(self, direction: Literal["forward", "back"],
        ...
        """
        initial_state = self.get_source_elts() if direction == "forward" else self.get_sink_elts()
        to_keep = self.get_reachable_elts(initial_state=initial_state, direction=direction, how=how)
    ...
    def from_op(storage: "Storage", f: Op) -> "ComputationFrame":
        call_hids = storage.call_storage.execute_df(f'SELECT call_history_id FROM calls WHERE op="{f.name}"')['call_history_id'].values.tolist()
        calls = {call_hid: storage.get_call(call_hid, lazy=True) for call_hid in call_hids}
        ...
        res = ComputationFrame(refs={}, calls={}, vs={}, fs={}, refinv={}, callinv={}, inp={}, out={}, creator={}, consumers={}, storage=storage)
    ...
    def get_graph_desc(self) -> str:
        ...
        isolated_vars = {vname for vname in self.vnames if not self.in_edges(vname) and not self.out_edges(vname)}
        if isolated_vars:
        ...
        for fname in self.sort_nodes(self.fnames):
            ...
            rhs = ", ".join([f"{k}={' | '.join(input_edges[k])}" for k in input_edges.keys() if len(input_edges[k]) > 0])
    ...
    def var_info(self, vname: str):
        ...
        for (fname, _, output_name) in self.in_edges(node=vname):
            creator_call_hids = self.get_adj_elts_edge(edge=(fname, vname, output_name), hids=self.vs[vname], direction="back")
            creator_parts.append(f'{vname}={fname}::{output_name} ({len(creator_call_hids)} calls)')
        creator_desc = f"Creator ops: {' | '.join(creator_parts)}"
        consumer_parts = []
        for (_, fname, input_name) in self.out_edges(node=vname):
            consumer_call_hids = self.get_adj_elts_edge(edge=(vname, fname, input_name), hids=self.vs[vname], direction="forward")
            consumer_parts.append(f'{fname}({input_name}={vname}, ...) ({len(consumer_call_hids)} calls)')
    ...
    def info(self, *nodes: str):
        if len(nodes) > 0: # we want info about specific nodes
            ...
            try:
                ...
                p = Panel(
                    Syntax(self.get_graph_desc(), "python",
                            # choose a theme like solarized-light
                            theme="solarized-light"
                            ), title=None, expand=True
                )
